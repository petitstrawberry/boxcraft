{
  description = "Boxcraft voxel game development environment";
  ...
  outputs = { self, nixpkgs, scarlet-rust-toolchain, scarlet-sdk }:
    let
      ...
      mkSystem = system:
        let
          ...
          imageTools = [ pkgs.coreutils pkgs.e2fsprogs pkgs.mtools ];
          ...
          runtimeTools = [ rustToolchain cargo-scarlet-plugin-limine pkgs.git pkgs.pkg-config pkgs.curl ] ++ imageTools;
        ...
        in {
          packages = { inherit cargo-scarlet cargo-scarlet-plugin-limine; default = cargo-scarlet; };
          devShell = pkgs.mkShell {
            packages = [ rustToolchain cargo-scarlet cargo-scarlet-plugin-limine pkgs.git pkgs.pkg-config pkgs.curl ] ++ imageTools;
            shellHook = ''
    ...
    in {
      ...
      devShells = forAllSystems (system: { default = (mkSystem system).devShell; });
    };
}
